...
class DiscreteBayesFilter:
    def __init__(self, floor_map, config: dict, motion_cfg: dict, initial_state: dict):
        ...
        # BELANGRIJK: grid nu in wereldcoördinaten mét origin
        self.xs = np.arange(
            floor_map.origin_x_m,
            floor_map.origin_x_m + floor_map.width_m + self.dx,
            self.dx,
        )
        self.ys = np.arange(
            floor_map.origin_y_m,
            floor_map.origin_y_m + floor_map.height_m + self.dy,
            self.dy,
        )
    ...
    def _precompute_map_likelihood(self):
        ...
        for iy, y in enumerate(self.ys):
            for ix, x in enumerate(self.xs):
                out[iy, ix] = self.floor_map.get_probability(x, y)
    ...
    def update_step(self, heading_change: float, step_length_m: float, dt: float = 1.0):
        ...
        effective_turn = self.turn_gain * self.directional_persistence * heading_change
        ...
        return {
            "position": mean,
            "variance": var,
            "neff": float(np.count_nonzero(self.belief > 1e-12)),
        }
